...
async function crearReporte(req, res) {
  try {
    ...
    const inicioReporte = moment(fechaInicio, "YYYY-MM-DD").startOf("day");
    const finReporte = moment(fechaFin, "YYYY-MM-DD").endOf("day");
    ...
    const deleteQuery = { 
      fechaInicioReporte: { $gte: inicioReporte.toDate() }, 
      fechaFinReporte: { $lte: finReporte.toDate() } 
    };
    ...
    if (todosLosFuncionarios.length === 0) {
      return res.json({ 
        success: true, 
        data: [], 
        mensaje: "No se encontraron funcionarios activos." 
      });
    }
    ...
    // CORRECCIÓN: Buscar turnos que SE SUPERPONEN con el rango de fechas
    ...
    res.status(500).json({ 
      success: false, 
      mensaje: "Error creando el reporte", 
      error: err.message 
    });
  }
}
...
async function exportarReporteExcel(req, res) {
  try {
    ...
    // Buscar turnos que SE SUPERPONEN con el rango de fechas
    const filtroHorasExtras = {
      FuncionarioAsignado: { $in: idsDeFuncionarios },
      fecha_inicio_trabajo: { $lte: finReporte.toDate() },
      fecha_fin_trabajo: { $gte: inicioReporte.toDate() }
    ...
    if (reportesFinales.length === 0) {
      return res.status(404).json({ 
        mensaje: "Ningún funcionario registró horas en el período seleccionado." 
      });
    }
    ...
    reportesFinales.forEach((r, index) => {
      ...
      const dataRow = worksheet.addRow([
        ...
        minutosAHHMM(r.HNF), 
        minutosAHHMM(r.RNO),
        parseFloat((r.HEDO / 60).toFixed(2)), 
        parseFloat((r.HENO / 60).toFixed(2)),
        parseFloat((r.HEDF / 60).toFixed(2)), 
        parseFloat((r.HENF / 60).toFixed(2)),
        parseFloat((r.HDF / 60).toFixed(2)), 
        parseFloat((r.HNF / 60).toFixed(2)),
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=Reporte_${tipoOperario || 'Todos'}_${fechaInicio}_${fechaFin}.xlsx`
    );
    ...
    res.status(500).json({ 
      mensaje: "Error exportando el reporte a Excel", 
      error: err.message 
    });
  }
}
